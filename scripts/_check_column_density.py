...
def parse_techs():
    src = DATA.read_text()
    # Strict pattern; landmark detection is a separate scan below.
    pat = re.compile(
        r'\{\s*id:\s*"([^"]+)"\s*,'
        r'\s*name:\s*"[^"]*"\s*,'
        r'\s*era:\s*"([^"]+)"\s*,'
    ...
    # Independently scan for `id: "X" ... landmark: true` within a single tech entry block.
    landmark_pat = re.compile(r'id:\s*"([^"]+)"[^{}]*?landmark:\s*true', re.S)
    landmarks = set(landmark_pat.findall(src))
    for t in techs:
        if t["id"] in landmarks:
            t["landmark"] = True
