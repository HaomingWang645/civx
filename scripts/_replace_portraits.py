#!/usr/bin/env python3
"""Replace human-portrait images with non-portrait alternatives. The user's
rule: never use a person's headshot/portrait as the lead image for a tech
node. Prefer artifacts, instruments, diagrams, events, or iconic works.

Re-runnable. Reads _image_manifest.json, downloads new thumbnails for the
listed techs, rewrites those entries, and regenerates images.js."""
...
# tech-id -> Wikipedia article title to use instead of the current portrait.
# Each replacement is an artifact, instrument, diagram, named work, event,
# or institution — never a person's article.
REPLACEMENTS = [
    ("modern-bookkeeping",             "Audit"),
]
...
def fetch_one(tid, article):
    ...
    if not thumb:
        return None, article_title, page_url, "no thumbnail", None
...
def main():
    ...
    failed = []
    for tid, article in REPLACEMENTS:
        try:
            img_path, art, page, err, summary = fetch_one(tid, article)
            if err:
                print(f"  [no-image] {tid:35} ({art}) — leaving original")
                failed.append((tid, article, err))
                time.sleep(0.5)
                continue
            manifest[tid] = {
                "name": tid, "query": article, "article": art,
                "page": page, "image_url": (summary.get("thumbnail") or {}).get("source"),
                "image_path": str(img_path.relative_to(ROOT)),
                "extract": (summary.get("extract") or "")[:280],
                "status": "ok", "score": 0.9, "reason": "portrait replacement",
            }
            print(f"  [ok]       {tid:35} -> {art}")
    ...
    # Regen images.js from full manifest
    entries, attribution = {}, {}
    ...
    if failed:
        print(f"\n{len(failed)} replacements failed (need second-pass alternates):")
