#!/usr/bin/env python3
"""Refetch Wikipedia thumbnails for 19 near/far-future techs whose current
images are missing, are portraits (forbidden), or point to misaligned articles.

For each tech we provide a *fallback chain* of candidate articles and stop at
the first one that yields a usable image. A result is rejected if:
  - the resolved article looks like a biography (description contains 'born YYYY'),
    because the no-portraits rule forbids using a person's headshot.
  - the only available media is math-rendered SVG (math equations).

Lookup order per article:
  1. /page/summary  -> thumbnail / originalimage
  2. /page/media-list -> first non-math image (srcset or original.source)

Existing manifest entries are preserved if all candidates fail.
"""
...
# Each tech maps to a priority-ordered list of candidate articles.
TARGETS = [
    ("neuromorphic-chip",            ["Neuromorphic engineering", "Memristor", "Spiking neural network"]),
    ("sentientism",                  ["Animal rights"]),
    ("formal-mathematics",           ["Coq (software)", "Lean (proof assistant)", "Proof assistant"]),
    ("superintelligence",            ["Superintelligence: Paths, Dangers, Strategies"]),
    ("direct-ai-democracy",          ["Pol.is", "E-democracy", "Civic technology", "Open government"]),
    ("agi",                          ["ChatGPT", "GPT-4", "Large language model"]),
    ("ai-governance",                ["Government by algorithm", "Algorithmic regulation"]),
    ...
    ("p-vs-np-resolved",             ["P versus NP problem"]),
    ("genetic-disease-eradication",  ["Gene therapy"]),
    ("relativistic-kinetic-weapon",  ["Kinetic bombardment", "Railgun", "Mass driver"]),
    ("reversible-computing",         ["Reversible computing", "Landauer's principle", "Maxwell's demon", "Adiabatic circuit"]),
    ("hypercomputation",             ["Hypercomputation", "Halting problem", "Oracle machine", "Computability theory"]),
]
...
def try_one_candidate(tid, article):
    """Returns (img_path, art_title, page_url, summary, used_fallback) or raises/returns reason."""
...
def main():
    ...
    for tid, candidates in TARGETS:
        ...
        manifest[tid] = {
            ...
            "status": "ok", "score": 0.9,
            "reason": "future realign batch (May 2026)" + (" [media-list]" if fb else "") + (f" [fallback after: {[a for a,_ in attempts]}]" if attempts else ""),
        }
        time.sleep(0.6)
